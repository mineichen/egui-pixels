...
pub(super) fn ui(ui: &mut egui::Ui, img: &ImageLoadOk, core: &mut egui_pixels::Tools) {
    ui.horizontal(|ui| {
        ui.label("Primary:");
        let mut active_idx = core.active_primary_idx();
        egui::ComboBox::from_id_salt("primary_tool")
            .selected_text(core.active_primary_name())
            .show_ui(ui, |ui| {
                for (i, name) in core.tool_names().enumerate() {
                    ui.selectable_value(&mut active_idx, i, name);
                }
            });
        core.set_primary_idx(active_idx, &img);
    });

    ui.horizontal(|ui| {
        ui.label("Secondary (CTRL):");
        let mut active_idx = core.active_secondary_idx();
        egui::ComboBox::from_id_salt("secondary_tool")
            .selected_text(core.active_secondary_name())
            .show_ui(ui, |ui| {
                for (i, name) in core.tool_names().enumerate() {
                    ui.selectable_value(&mut active_idx, i, name);
                }
            });
        core.set_secondary_idx(active_idx, img);
    });
}
